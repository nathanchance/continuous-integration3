...
def create_and_configure_user() -> None:
    try:
        ...
        print(f"[-] {RUNNER_NAME} account already exists, skipping...")
        return
    ...
    subprocess.run(['loginctl', 'enable-linger', RUNNER_NAME], check=True)
...
def register_gh_runner() -> None:
    ...
    setup_cmds = [
        ...
        f"./config.sh --url https://github.com/{repo} --token {token} --labels {runner_name.rsplit('-', 2)[1]} --unattended",
    ]
