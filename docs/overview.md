...
- A virtual machine to...
    - Mirror git repositories
        - kernel sources from `git.kernel.org` using [`grokmirror`](https://github.com/mricon/grokmirror) (refreshed every ten minutes)
        - [boot-utils](https://github.com/ClangBuiltLinux/boot-utils) (refreshed every fifteen minutes)
...
- `docs/`: Documentation (most relevant for maintainers)
- `generator/`: Generator to turn `.toml` build descriptions into `.github/workflows/generated-*.yml` files
...
- Generate [a fine-grained access token](https://github.com/settings/personal-access-tokens/new) to allow programmatically adding and deleting self-hosted runners on each build host
    ```
...
5. Create virtual machines using `infra/vmm.py`
    ```
    run0 infra/vmm.py create -m -n2 -b1 -g
    ```
    The above command creates a mirror virtual machine (`-m`), two normal virtual machines (`-n2`), and one big virtual machine (`-b1`) while automatically passing through the fine-grained access token created above from the host to the virtual machine for automated registration via the GitHub REST API (`-g`).
